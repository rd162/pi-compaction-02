...
```bash
pi install git:rd162/pi-compaction
# or npm, once published:
pi install npm:pi-compaction
```
...
```bash
node test/muse-compaction-test.mjs      # 24 unit tests
node test/muse-compaction-stress.mjs    # 900-msg hostile spans, determinism
node test/muse-compaction-stress2.mjs   # 56 exotic-tool commands (jq/jd/difft/bat/eza/sqlite3/ast-grep/ffmpeg…)
node test/muse-compaction-100k.mjs      # 100k tool pairs, budget exhaustion
```
